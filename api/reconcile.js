...
async function fetchShopifyOrders(createdAtMin) {
  const orders = [];
  let url = `https://${STORE_DOMAIN}/admin/api/2026-04/orders.json?created_at_min=${createdAtMin}&limit=250&status=any`;
...
async function sendOrderWhatsApp(order, orderData) {
  ...
  for (const item of line_items) {
    if (!seenIds.has(item.product_id)) {
      try {
        const res = await fetch(
          `https://${STORE_DOMAIN}/admin/api/2026-04/products/${item.product_id}.json?fields=image,title`,
